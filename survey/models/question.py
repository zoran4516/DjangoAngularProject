...
class Question(models.Model):
    ...
    QUESTION_TYPES = (
        (PARA, _(u'text (multiple line)')),
        (SHORT, _(u'short text (one line)')),
        (RADIO, _(u'radio')),
        (CHECKBOX, _(u'checkbox')),
        (INTEGER, _(u'integer')),
        (DROPBOX, _(u'dropbox')),
        (DATE, _(u'date')),
        (TIME, _(u'time')),
    ...
    order = models.IntegerField(_("Order"), default=1, blank=True, null=True)
    required = models.BooleanField(_("Required"),)
    category = models.ForeignKey(Category, verbose_name=_("Category"), blank=True, null=True, related_name="questions")
    survey = models.ForeignKey(Survey, verbose_name=_("Survey"), related_name="questions")
    type = models.CharField(_("Type"), max_length=200, choices=QUESTION_TYPES, default=PARA)
    choices = models.TextField(_("Choices"), blank=True, null=True, help_text=CHOICES_HELP_TEXT)
    placeholder = models.CharField(_("Placeholder"), default="", blank=True, null=True, max_length=1000,)
    min = models.IntegerField(_("Min"), default=0, blank=True, null=True)
    max = models.IntegerField(_("Max"), default=2000, blank=True, null=True)
    tabs = models.IntegerField(_("Tabs"), default=-1, blank=True, null=True)
    dependent_question = models.TextField(_("dependentQuestion"), blank=True, null=True)
    dependent_option = models.TextField(_("dependentOption"), blank=True, null=True)
    tab_names = models.TextField(_("tabNames"), blank=True, null=True)
    admin_question = models.BooleanField(default=False, blank=True)

    is_tricky = models.BooleanField(default=False, blank=True)
    tricky_id = models.IntegerField(default=0, blank=True, null=True)
    ...
    def save(self, *args, **kwargs):
        if self.type in [Question.RADIO, Question.RADIO,
                         Question.CHECKBOX]:
