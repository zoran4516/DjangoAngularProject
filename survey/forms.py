...
from .models import Answer, Question, Responses
...
class ResponseForm(models.ModelForm):
    ...
    WIDGETS = {
        Question.PARA: forms.Textarea,
        Question.SHORT: forms.TextInput,
    ...
    def _get_preexisting_response(self):
        ...
        if not self.user.is_authenticated():
            return None
        try:
            return Responses.objects.get(user=self.user, survey=self.survey)
    ...
    def get_question_choices(self, question):
        ...
        qchoices = None
        if question.type not in [Question.PARA, Question.SHORT,
                                 Question.INTEGER]:
    ...
    def get_question_field(self, question, **kwargs):
        ...
        FIELDS = {
            Question.PARA: forms.CharField,
            Question.SHORT: forms.CharField,
            Question.CHECKBOX: forms.MultipleChoiceField,
    ...
    def save(self, commit=True):
        ...
        for field_name, field_value in self.cleaned_data.items():
            if field_name.startswith("question_"):
                ...
                answer.save()
        survey_completed.send(sender=Responses, instance=response, data=data)
        return response
