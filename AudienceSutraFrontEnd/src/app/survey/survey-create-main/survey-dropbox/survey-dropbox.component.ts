...
@Component({
  selector: 'app-survey-dropbox',
  templateUrl: './survey-dropbox.component.html',
  styleUrls: ['./survey-dropbox.component.css']
})
export class SurveyDropboxComponent implements OnInit {
  @Input() group: any;
  @Input() index: number;
  ...
  generateTabs() {
    const temp = [];
    const tabsLength = this.parentGroup.get('tabs') as FormArray;
    for (let i = 1; i <= tabsLength.length; i++) {
      temp.push(i);
    }
    return temp;
  }
